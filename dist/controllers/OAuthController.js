...
class OAuthController {
    ...
    tokenService;
    userRepo;
    constructor(authService, tokenService, userRepo) {
    ...
    authorize = async (req, res, next) => {
        try {
            ...
            const client = await (0, database_1.queryOne)('SELECT * FROM oauth_clients WHERE client_id = ?', [client_id]);
            if (!client) {
            ...
            if (req.session?.userId) {
                ...
                await (0, database_1.query)(`INSERT INTO oauth_authorization_codes (code, client_id, user_id, redirect_uri, code_challenge,
            code_challenge_method, scopes, expires_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, DATE_ADD(NOW(), INTERVAL ? SECOND))`, [code, client.client_id, user.id, redirect_uri, code_challenge || null,
            ...
            await (0, database_1.query)(`INSERT INTO oauth_authorization_codes (code, client_id, user_id, redirect_uri, code_challenge,
          code_challenge_method, scopes, expires_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, DATE_ADD(NOW(), INTERVAL ? SECOND))`, [code, client.client_id, result.user.id, redirect_uri, code_challenge || null,
                code_challenge_method || null, JSON.stringify([]), CODE_TTL]);
            return res.redirect(`${redirect_uri}?code=${code}&state=${state || ''}`);
        }
        catch (err) {
    ...
    token = async (req, res, next) => {
        try {
            ...
            if (grant_type === 'authorization_code') {
                ...
                const storedCode = await (0, database_1.queryOne)('SELECT * FROM oauth_authorization_codes WHERE code = ? AND client_id = ? AND used = 0 AND expires_at > NOW()', [code, client_id]);
                if (!storedCode) {
                    return res.status(400).json({ error: 'invalid_grant' });
                }
                if (storedCode.code_challenge && code_verifier) {
                    const verifierHash = crypto_1.default
                        .createHash('sha256')
                ...
                await (0, database_1.query)('UPDATE oauth_authorization_codes SET used = 1 WHERE code = ?', [code]);
                const user = await this.userRepo.findById(storedCode.user_id);
                if (!user || user.status !== 'active') {
                    return res.status(400).json({ error: 'invalid_grant' });
                }
                const accessToken = this.tokenService.generateAccessToken(user);
                const refreshTokenValue = crypto_1.default.randomUUID();
                const tokenHash = crypto_1.default.createHash('sha256').update(refreshTokenValue).digest('hex');
                await (0, database_1.query)(`INSERT INTO refresh_tokens (user_id, token_hash, device, expires_at)
           VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 604800 SECOND))`, [user.id, tokenHash, req.headers['user-agent'] || null]);
                return res.json({
            ...
            if (grant_type === 'refresh_token') {
                ...
                const stored = await (0, database_1.queryOne)('SELECT * FROM refresh_tokens WHERE token_hash = ? AND revoked = 0 AND expires_at > NOW()', [tokenHash]);
                if (!stored) {
                    return res.status(400).json({ error: 'invalid_grant' });
                }
                ...
                await (0, database_1.query)(`INSERT INTO refresh_tokens (user_id, token_hash, device, expires_at)
           VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 604800 SECOND))`, [user.id, newHash, stored.device]);
                return res.json({
                    access_token: accessToken,
